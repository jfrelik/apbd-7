...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(emp => Emps.Max(e => e.Salary) == emp.Salary);
            return result;
        }
        ...
        public static IEnumerable<object> Task7()
        {
            IEnumerable<object> result = Emps
                .Select(emp => new { Praca = emp.Job, LiczbaPracownikow = Emps.Count(e => e.Job == emp.Job) })
                .Distinct();
            return result;
        }
        ...
        {
            int result = arr.GroupBy(i => i).Where(g => g.Count() % 2 != 0).Select(g => g.Key).First();
            //result=
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Where(d => Emps.Count(e => e.Deptno == d.Deptno) == 5 || Emps.Count(e => e.Deptno == d.Deptno) == 0)
                .OrderBy(d => d.Dname);
            //result =
            return result;
        }
